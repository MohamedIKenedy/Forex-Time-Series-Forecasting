...
<!-- Moved: see etl/docs/etl_yfinance_s3_spark_airflow_postgres.md -->
This document has been moved to `etl/docs/etl_yfinance_s3_spark_airflow_postgres.md`.
Please consult that file for the ETL documentation.
...
- `api/services/yfinance_s3.py` — fetcher that downloads data via `yfinance` and uploads raw CSV to S3.
- `api/jobs/spark_transform.py` — PySpark job to read raw CSVs from S3, normalize schema and write partitioned Parquet to S3.
- `airflow/dags/forex_yfinance_etl.py` — Airflow DAG orchestrating the fetch → spark → load → validate flow.
- `api/services/load_to_sql.py` — loader with an idempotent upsert implementation for Postgres (staging + INSERT ... ON CONFLICT).
- `api/services/validation.py` — validation helpers (counts, null checks, continuity) plus SNS alerting.
...
```powershell
python -m api.services.yfinance_s3 EURUSD=X --start 2024-01-01 --end 2025-01-01 --bucket my-bucket --prefix forex
```

4. Run the Spark job locally with `spark-submit` (ensure Hadoop AWS jars on classpath if using S3):

```bash
spark-submit api/jobs/spark_transform.py --input s3a://my-bucket/forex/raw/... --output s3a://my-bucket/forex/processed/2025-01-01/
```
...
- [api/services/yfinance_s3.py](api/services/yfinance_s3.py) — fetch + S3 upload
- [api/jobs/spark_transform.py](api/jobs/spark_transform.py) — PySpark transform job
- [airflow/dags/forex_yfinance_etl.py](airflow/dags/forex_yfinance_etl.py) — DAG wiring and XCom usage
- [api/services/load_to_sql.py](api/services/load_to_sql.py) — upsert loader (staging + ON CONFLICT)
- [api/services/validation.py](api/services/validation.py) — validation and SNS alerting
...
- produce a README-style quickstart that automates creating Airflow Variables and Connections for this pipeline, or
- add a Redshift COPY implementation and manifest generator for high-volume loads.
