...
class FeatureStore:
    ...
    def fetch_realtime_data_from_kafka(
        self, ticker: str, max_messages: int = 500, timeout_ms: int = 10000
    ) -> pd.DataFrame:
        """
                            ...
                            data.append(
                                {
                                    ...
                                    "Volume": int(
                                        record.get("volume", record.get("Volume", 0))
                                    ),
                                }
                            )
                            msg_count += 1
                ...
                if not data:
                    try:
                        ...
                        for message in consumer:
                            record = message.value
                            if record.get("ticker") == ticker:
                                date_field = record.get("timestamp") or record.get(
                                    "date"
                                )
                                if date_field:
                                    data.append(
        ...
        if not data:
            raise ValueError(
                f"No real-time data found for {ticker} in any Kafka streaming topic"
            )
    ...
    def create_lagged_features_for_inference(
        self, df_history: pd.DataFrame, lookback: int = 200
    ) -> pd.DataFrame:
        """
        ...
        for window in [5, 10, 20, 60]:
            if window <= lookback and "Close_log_return" in features.columns:
                ...
                features[f"close_log_return_mean_{window}"] = shifted.rolling(
                    window
                ).mean()
                features[f"close_log_return_std_{window}"] = shifted.rolling(
                    window
                ).std()
    ...
    def get_inference_features(
        self, ticker: str, lookback: int = 200
        ...
        """
        try:
            df_history = self.fetch_realtime_data_from_kafka(
                ticker, max_messages=lookback + 250
            )
            ...
            features_full = self.create_lagged_features_for_inference(
                df_filtered, lookback=lookback
            )
