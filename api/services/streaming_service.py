                ...
                try:
                    ...
                    try:
                        data = ticker_obj.history(
                            period=used_period, interval=used_interval
                        )
                    except:
                        data = None
                    ...
                    if data is not None and not data.empty:
                        ...
                        if (
                            last_key
                            and last_key[0] == ts_str
                            and abs(last_key[1] - latest_close) < 1e-12
                        ):
                            pass
                        else:
                            message = {
                            ...
                            self.kafka_service.produce_message(
                                topic=topic, message=message, key=ticker
                            )
                        ...
                        print(
                            f"[WARNING] No data available for {ticker} - market may be closed"
                        )

                    # Try to get a fresher quote price to update intraminute.
                    quote_price = get_quote_price(ticker_obj)
                    if quote_price is not None and (
                        latest_close is None or abs(quote_price - latest_close) > 0
                    ):
                        now_ts = (
                        ...
                        self.kafka_service.produce_message(
                            topic=topic, message=quote_msg, key=ticker
                        )
                        if on_message_callback:
                            on_message_callback(
                                ticker, quote_msg, period=used_period, interval="tick"
                            )
            ...
            for ticker in tickers:
                ...
                try:
                    ...
                    if data is not None and not data.empty:
                        ...
                        if on_message_callback:
                            on_message_callback(
                                ticker, message, period="1d", interval="1h"
                            )
                    else:
                        print(
                            f"[WARNING] No data available for {ticker} - market may be closed"
            ...
            for ticker in tickers:
                ...
                try:
                    ...
                    if not data.empty:
                        ...
                        if on_message_callback:
                            on_message_callback(
                                ticker, message, period="60d", interval="1d"
                            )
                except Exception as e:
                    print(f"[ERROR] Failed to fetch {ticker}: {e}")
